...
void arducam_capture_frame(struct arducam_config *config, uint8_t* image) {
	...
	uint8_t temp = 0;
	  for (y = 24; y < 216; y+=2) 
	  {
		...
		{
			image_tmp[index++] = config->image_buf[y * 320 + x];
		}
	  } 
		index = 0;
		for (y = 0; y < 96; y++) 
	  {
			for (x = 0; x < 96; x++) 
			{
				uint16_t rgbData= ST7735_COLOR565(image_tmp[y*96+x],image_tmp[y*96+x],image_tmp[y*96+x]);
				image[index++] = (uint8_t)(rgbData>>8 )&0xFF;
				image[index++] = (uint8_t)rgbData&0xFF;
			}
	  }
#endif
